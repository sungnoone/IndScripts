...
/*
app.scriptArgs.set( "setPageStartRange" , "1" );
app.scriptArgs.set( "setPageEndRange" , "18" );
app.scriptArgs.set( "setStart_y1" , "0" );
app.scriptArgs.set( "setStart_y2" , "1055" );
app.scriptArgs.set( "setFontSize" , "13" );
app.scriptArgs.set( "答案C" , "" );
*/
...
function catchText(){
    nowDate = new Date();
    var outputFolderName = Folder.desktop.fsName + "\\" + inDoc.name + "_output"
    ...
    var outputFileName = outputFolderName+"\\"+inDoc.name + ".txt"; // 轉出文字檔以InDesign文件名稱為預設檔名
    checkDestFile (outputFileName); // 寫入日期戳記
    ...
    for( var v1=setPageStartRange-1 ; v1<setPageEndRange ; v1++ ){
        var myPage = inDoc.pages[v1];
        var pgSection = myPage.appliedSection; // 章節
        var pgName = pgSection.name + myPage.name;// 拼湊頁俗名        
        longStr = "";
        longStr += "\n========== Page "+(v1+1)+ "\tName:" + pgName +" ==========\n\r";        
        ...
        if( tfAry!=null ){
            if( necessaryBool1==false && necessaryBool3==false ){ // 字體大小、顏色條件都沒指定
                // 都沒設定條件 範圍內就抓
...
//檢查輸入頁碼參數是否合理......................................................................
function checParas_PageNo(){
    // 起始頁傳入參數小於0或大於總頁數，均為不合理
    if( setPageStartRange<0 || setPageStartRange>inDoc.pages.length ){
        return false;
    }
    // 結束頁傳入參數小於0或大於總頁數，均為不合理
    if( setPageEndRange<0 || setPageEndRange>inDoc.pages.length ){
        alert();
        return false;
    }
    return true; // 通過考驗才傳回true
}
...
function setFindPrefs_FillColor( ColorName ){
	...
	var appColor = null;
    for( v1=0 ; v1<app.colors.length ; v1++ ){
        if( app.colors[v1].name == ColorName ){
            //有同名色票情況
            appColor = app.colors.itemByName(ColorName); // 先取得同名舊色票
            // App中有同名的先殺掉
            try{
                // 嘗試殺掉 以相同屬性再建一個( 因有舊設定遺留但顏色內容不相同疑慮 )                
                appColor.remove();
                appColor = app.colors.add( docColor.properties ); // 以相同屬性加色票
            }catch(e){                
                //若未能成功建立
                //可能是系統色票，直接沿用
                appColor = app.colors.itemByName(ColorName);
             }
            findTextPrefs.fillColor = appColor; // 設定搜尋色票
            return;// 離開迴圈
        }
    }
    // 比對迴圈完畢，都沒有同名色票存在情況
...
// 判斷檔案是否存在，如已存在，結果附加到文字檔內，如不存在，新增檔案......................................................................   
function checkDestFile(filePathStr){
    var txtFile = new File (filePathStr);
